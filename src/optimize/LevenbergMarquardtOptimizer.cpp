...
//   DEBUG_MSG("Print alpha to see whether it is changing");
//   for (size_t i = 0; i < max_iter; i++) {
...
//   //Cache old gradient vector and calculate new one
//   Eigen::Matrix<double, Eigen::Dynamic, 1> vec_old = vec;
//   vec = jacobian.transpose() * residual;

//   //Determine new lambda parameter from new and old gradient vector
//   //If done like this lambda is updated independendtly on whether the residual norm decreases (CHECK)
//   if (!first_step) {
...
void LevenbergMarquardtOptimizer::update_delta(bool /*first_step*/) {
  ...
  // Solve for delta robustly (try LLT then fallback)
  //Eigen::LLT<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>> llt(mat);
  //if (llt.info() == Eigen::Success) {
    //delta = llt.solve(vec);
  //} else {
    // fallback to a more robust solver if mat not PD
    //delta = mat.colPivHouseholderQr().solve(vec);
  //}
}
