...
Eigen::Matrix<double, Eigen::Dynamic, 1> LevenbergMarquardtOptimizer::run(
  ...
  // Open file to log trajectory (once)
  std::ofstream traj_file("lm_trajectory_2.csv");
  traj_file << "iter,lambda,cost";
  ...
  for (size_t i = 0; i < max_iter; ++i) {
    ...
    double cost_iter = 0.5 * residual.squaredNorm();
    ...
    // Log the accepted state of this iteration - to plot optimization trajectory
    traj_file << i << "," << lambda << "," << cost_iter;
    for (int p = 0; p < num_params; ++p) {
      traj_file << "," << alpha[p];
    }
    traj_file << "\n";
